...
class View:
    ...
    states = ["WaitingForPlayer", "GettingPlayerMove", "ProcessingPlayerMove", "GettingComputerMove", "MakingComputerMove", "GameOver"]
    revertStates = {
        ...
        for i in range(8):
            for j in range(8):
                if (i + j) % 2 == 0:
                    ...
                    color = (0, 0, 0)
                pg.draw.rect(self.board, color, (i * SQUARE_SIZE, j * SQUARE_SIZE,
    ...
    def gameLoop(self):
        ...
        while running:
            ...
            if not self.state == self.previousState:
                ...
                match self.state:
                    case 'WaitingForPlayer':
                        ...
                        currentAwait = None
                    case 'GettingPlayerMove':
                        currentAwait = ThreadWithReturn(self.gl.getPlayerMove)
                        currentAwait.start()
                    case 'ProcessingPlayerMove':
                        currentAwait = ThreadWithReturn(self.gl.processPlayerMove, previousResult)
                        currentAwait.start()
                    case 'GettingComputerMove':
                        currentAwait = ThreadWithReturn(self.gl.generateComputerMove)
                        currentAwait.start()
                    case 'MakingComputerMove':
                        currentAwait = ThreadWithReturn(self.gl.makeComputerMove, previousResult)
                        currentAwait.start()
                    case 'GameOver':
                        currentAwait = None
                ...
                if currentAwait is None:
                    ...
                    if not previousResult:
                        if self.state in self.revertStates:
                            self.logger.error("Error in {}, restarting from {}".format(self.state, self.revertStates[self.state]))
                            self.state = self.revertStates[self.state]
                            currentState = self.states.index(self.state)
                        else:
                            self.logger.warning("Error in {}, restarting from beginning of state".format(self.state))
                            self.previousState = self.states[(currentState - 1) % len(self.states)]
                    else:
            ...
            self.screen.blit(self.defaultFont.render(f"Frame: {self.frame}", True, (0, 0, 0), (255, 255, 255)), (BOARD_WIDTH, 0))
            self.screen.blit(self.defaultFont.render(f"State: {self.state}", True, (0, 0, 0), (255, 255, 255)), (BOARD_WIDTH, 20))
